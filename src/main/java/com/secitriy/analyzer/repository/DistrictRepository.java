...
public interface DistrictRepository extends JpaRepository<District, Long> {
    ...
    @Query(
        value = "select district from District district left join fetch district.state",
        countQuery = "select count(district) from District district"
    )
    Page<District> findAllWithToOneRelationships(Pageable pageable);

    @Query("select district from District district left join fetch district.state")

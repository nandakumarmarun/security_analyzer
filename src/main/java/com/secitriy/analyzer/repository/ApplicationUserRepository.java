...
public interface ApplicationUserRepository extends JpaRepository<ApplicationUser, Long> {
    ...
    @Query(
        value = "select applicationUser from ApplicationUser applicationUser left join fetch applicationUser.internalUser left join fetch applicationUser.userAttributes",
    ...
    @Query(
        "select applicationUser from ApplicationUser applicationUser left join fetch applicationUser.internalUser left join fetch applicationUser.userAttributes"
    )
    List<ApplicationUser> findAllWithToOneRelationships();

    @Query(
        "select applicationUser from ApplicationUser applicationUser left join fetch applicationUser.internalUser left join fetch applicationUser.userAttributes where applicationUser.id =:id"
    )
